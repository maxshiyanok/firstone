...
class PostList{
	...
	edit(id,post){
			let ind = this._posts.findIndex(item => item.id == id);
			if(ind == -1)
				return false;
			let editPost = Object.assign({}, this._posts[ind]);
			for(let key in post){
	...
	remove(id){
			let ind = this._posts.findIndex(post => post.id == id);
			if(ind != -1)
				{
	...
	addOrRemoveLike(id, name){
			let likes = this.get(id).likes;
...
console.log(posts);
console.log(tweet.getPage(0,5,{author: 'Иванов Иван'}));
...
console.log(tweet.addOrRemoveLike('1','maxim'));
console.log(tweet.addOrRemoveLike('1','d'));
